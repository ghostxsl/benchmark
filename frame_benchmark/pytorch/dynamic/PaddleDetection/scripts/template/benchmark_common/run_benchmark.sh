...
function _set_params(){
    ...
    model_repo="template_repo"          # (必选) 模型套件的名字
...
function _train(){
    ...
    train_options="--no-validate \
                   --cfg-options log_config.interval=template_log_interval \
                   data.samples_per_gpu=${batch_size} \
                   runner.max_epochs=${max_epochs} \
                   data.workers_per_gpu=${num_workers} ${set_fp_item}"

    case ${run_process_type} in
    SingleP) train_cmd="python tools/train.py ${train_config} ${train_options} optimizer.lr=template_N1C1 " ;;
    MultiP) train_cmd="bash ./tools/dist_train.sh ${train_config} 8 ${train_options} optimizer.lr=template_N1C8 " ;;
